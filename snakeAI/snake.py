...
class Snake:
    ...
    def dfsSpaceLeft(self, direction):
        nextx = int(self.head[0]/10)
        nexty = int(self.head[1]/10)
        ...
        o = 0
        if (self.inBounds(nextx, nexty)):
            visited = [[0 for i in range (40)] for j in range(40)]
            c = 0
            o = self.dfs(visited, nextx, nexty, c)
    ...
    def dfs(self, a, x, y, count):
        ...
        if self.inBounds(x+1, y) and not [(x+1) * 10, y * 10] in self.snakeList and a[x+1][y] == 0:
            count += 1
            right = self.dfs(a, x+1, y, count)
        if self.inBounds(x+1, y) and not [(x-1) * 10, y * 10] in self.snakeList and a[x-1][y] == 0:
            count += 1
            left = self.dfs(a, x-1, y, count)
        if self.inBounds(x+1, y) and not [x * 10, (y+1) * 10] in self.snakeList and a[x][y+1] == 0:
            count += 1
            down = self.dfs(a, x, y+1, count)
        if self.inBounds(x+1, y) and not [x * 10, (y-1) * 10] in self.snakeList and a[x][y-1] == 0:
            count += 1
            up = self.dfs(a, x, y-1, count)

        out = up + down + left + right + 1
        if out > 10:
            out = 10
    ...
    def inBounds(self, x, y):
        if (x < 0 or x * 10 >= XSIZE) or (y < 0 or y * 10 >= YSIZE):
            return False
        else:
            return True
